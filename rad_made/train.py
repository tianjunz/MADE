...
class Workspace(object):
    def __init__(self, cfg):
        ...
        if cfg.agent.name == 'made':
            self.latest_replay_buffer = ReplayBuffer(
                self.env.observation_space.shape,
                self.env.action_space.shape,
                self.env.state_space.shape,
                cfg.latest_replay_buffer_capacity,
                self.cfg.image_size,
                None,
                self.cfg.aug_type,
                self.cfg.use_drq,
            ...
            self.latest_replay_buffer = None
        ...
        while self.step < self.cfg.num_train_steps:
            ...
            if self.step >= self.cfg.num_seed_steps:
                for _ in range(self.cfg.num_train_iters):
                    if self.latest_replay_buffer is not None:
                        self.agent.update(self.replay_buffer, self.latest_replay_buffer, self.logger, self.step)
                    else:
            ...
            if self.latest_replay_buffer is not None:
                self.latest_replay_buffer.add(obs, action, reward, next_obs, done, done_no_max)
...
@hydra.main(config_path="made.yaml", strict=True)
def main(cfg):
