...
def load_vae_episodes(directory, rescan, num_episodes, length=None, balance=False, seed=0):
    directory = pathlib.Path(directory).expanduser()
    random = np.random.RandomState(seed)
    cache = {}
    while True:
        for filename in directory.glob("*.npz"):
            if filename not in cache:
                try:
                    ...
                    continue
                cache[filename] = episode
        if len(list(cache.keys())) > num_episodes:
            keys = list(cache.keys())
            keys.sort()
...
def _cat_sample(self, sample_shape=(), seed=None, name="sample"):
    ...
    indices = tf.random.categorical(
        self.logits_parameter(),
        sample_shape[0] if sample_shape else 1,
        self.dtype,
        seed,
        name,
    )
